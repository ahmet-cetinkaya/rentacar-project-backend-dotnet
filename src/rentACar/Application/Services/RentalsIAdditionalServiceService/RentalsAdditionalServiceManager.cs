...
{
    private IRentalsAdditionalServiceRepository _additionalServiceRepository;
    ...
    public async Task<IList<RentalsAdditionalService>> AddManyByRentalIdAndAdditionalServices(int rentalId, IList<AdditionalService> additionalServices)
    {
        IList<RentalsAdditionalService> rentalsAdditionalServices = 
            additionalServices.Select(a=> new RentalsAdditionalService{RentalId = rentalId, AdditionalServiceId = a.Id}).ToList();

        foreach (var rentalsAdditionalService in rentalsAdditionalServices)
